...
void Robot::memorizeIntensity()
{
  ...
  double tmin = 50;
  int tcount = 0;
  while (!((0.9 < tmin) && (tmin < 1.1)) && (tcount < 3)) {
    tmin = 50;
    for (int i = center - search_lines/2; i <= center + search_lines/2; i++) {
      if (tmin > laser_distance[i])  tmin = laser_distance[i];
    }
    sprintf(str,"%.1fmeter",tmin);
    printf("%.1f\n",tmin);
    sc->say(str);
    sleep(3);
    if ((0.9 < tmin) && (tmin < 1.1)) tcount++;
    else tcount = 0;

    
    if (1.1 < tmin)      sc->say("close to me");
    else if (tmin < 0.9) sc->say("apart from me");
    else sc->say("OK");
  ...
  const double stand_distance_min  = 0.0;
  ...
  for (int k = 0; k < repeat_no; k++) {
    ...
    if (count < 50) k--; // if the number of data is less than 50, do it again
      ...
      printf("epoch=%2d line no=%d Intensity min=%.1f  max=%.1f ave=%.2f sigma=%.2f\n",
           k,count,min[k], max[k], ave[k], sigma[k]);
    }
    ros::spinOnce();
  ...
  min_med = (min_med * 255.0/6000);
  max_med = (max_med * 255.0/6000);
...
void Robot::recognizeVoice(const string str)
{
  ...
  while (recog == false) {
    ...
    if (state == STATE1) system(SPEAK_SAY_FOLLOWME);
    if (state == STATE2) system(SPEAK_SAY_LEAVE);
    sleep(4);
  ...
  //sc->startWave(chime);
  sleep(2);
  if (state == STATE1) system(SPEAK_FOLLOWYOU);
  if (state == STATE2) system(SPEAK_LEAVE);
  //sleep(2);
}
...
void Robot::memorizeOperator()
{
  ...
  //man.intensity_min =  40;
  //man.intensity_max = 190;
}
...
void Robot::followOperator(cv::Mat input_image)
{
  float leg_radius = 0.05 * mToPixel;
  Object obj[100], obj1[100], obj2[100];
  int obj_num = 0, obj1_num = 0, obj2_num = 0;
...
void Robot::init()
{
  ...
  //cmdVelPublisher = nh.advertise<geometry_msgs::Twist>("cmd_vel_mux/input/teleop", 1);
  ...
  //laserSubscriber  = nh.subscribe("/scan", 100, laserCallback);
...
int main(int argc, char* argv[])
{
  ...
  //robot.setState(STATE1);       // begin from STATE1
  //robot.setState(STATE3);       // begin from STATE3
  //robot.setState(STATE_TEST);   
  ...
  while(ros::ok()) {
    ...
    switch (robot.getState()) {
      ...
      case STATE1:      robot.followOperator(e1_img); break;
