 ...
 int Robot::navigation(MoveBaseClient *ac)
{
  ...
  while (waypoint[num][2] !=777) {
    ...
    while (!result) {
      ...
      bool flag = false;
      double dist_threshold = 0.25; // この距離内に入ったらウェイポイントに到達したとみなす
      double dir_threshold = 30*M_PI/180.0; // 角度

      //　ロボットがウェイポイントに近づき、その方向を向いた場合
      if ((num == 2) && (dist_threshold * 2.0 < dist) 
	  && (dist < dist_threshold * 6.0) && (diff_yaw < dir_threshold)) {
	// ウェイポイント２の周りには障害物がある
	...
	if ((dist <= dist_threshold)
	    && (diff_yaw <= dir_threshold)) { // M_PI/180
	  result = true;
	}
      }
      usleep(5*1000);
...
void sr_response( const std::string& msg )
{
   ...
   if (msg == "mini"){
     robot.recog_mini = true;
     sc->startWave(chime);
     sleep(3);
   }
   else {
     if (robot.recog_mini == true) {
       if (msg == "stop"){
	 sc->say("You said stop");
	 sleep(3);
	 robot.recog_stop = true;
	 robot.recog_mini = false;
       }
       else if (msg == "follow me"){
	 sc->say("follow me");
	 sleep(3);
	 robot.recog_followme = true;
	 robot.recog_mini = false;
       }
       else if (msg == "leave the elevator"){
	 sc->say("leave the elevator");
	 sleep(3);
	 robot.recog_leave = true;
	 robot.recog_mini = false;
       }
     }
     //usleep(10*1000);                                                               
...
void Robot::init()
{
  ...
  // Set the subscribers
  speechSubscriber = nh.subscribe("/recognizer/output",10,&Robot::speechCallback,this);
