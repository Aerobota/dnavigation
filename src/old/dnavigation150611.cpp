 ...
 int Robot::navigation(MoveBaseClient *ac)
{
  ...
  while (waypoint[num][2] !=777) {
    ...
    while (!result) {
      ...
      if ((num == 2) && dist > 0.5) { 
	// ウェイポイント２の周りには障害物がある
        if (1.0 < dist && dist < 1.2) {
	  //stop();
	  for (int i=0; i < 10; i++) {
	    prepWindow();
	    ros::spinOnce();
	    flag = findHuman(e1_img);
	    showWindow();
	    usleep(30*1000);
	  }
	}
	...
	if (dist <= 0.25 && diff_yaw <= 0.6) { // M_PI/180
	  result = true;
	}
      }
      usleep(5*1000);
	...
	//usleep(2000*1000);
	switch (num) {
	case 1:system(SPEAK_WAYPOINT1); break;
	case 2:system(SPEAK_WAYPOINT2); break;
	case 3:system(SPEAK_WAYPOINT3); break;
	case 4:system(SPEAK_WAYPOINT4); break;
	case 5:system(SPEAK_WAYPOINT5); break;
	}
	usleep(1000*1000);
      }
    }
