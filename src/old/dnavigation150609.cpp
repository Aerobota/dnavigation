...
void Robot::memorizeIntensity()
{
  ...
  int val2 = system(SPEAK_REMEMBERED);
}
...
void Robot::searchOperator()
{
  ...
  while (1) {
	...
	int val = system(SPEAK_WALK_IN_PLACE);
	sleep(1);
	//state = STATE1;
	return;
      }
...
void Robot::searchOperatorMarco()
{
  ...
  stop();
  int val1 = system(SPEAK_WALK_IN_PLACE);
  sleep(5);
  int val2 = system(SPEAK_FIND_OPERATOR);
  sleep(5);
...
{
  ::voice_command = voice;
  std::cout << "speechCB:" << voice << std::endl;
  cout << "recog_kenseiko:" << recog_kenseiko << endl;
  cout << "recog_followme:" << recog_followme <<endl;

  /* if (::voice_command.data == "kenseikochan"){
    recog_kenseiko = true;
    //recog_followme = false;
    //recog_leave    = false;
    return;
  ...
  if (::voice_command.data == "follow me"){
    recog_followme = true;
    //recog_kenseiko = false;
    //recog_leave    = false;
    return;
  ...
  if (::voice_command.data == "leave the elevator"){
    recog_leave = true;
    //recog_kenseiko = false;
    //recog_followme = false;
    return;
...
void Robot::odomCallback(const nav_msgs::OdometryConstPtr& msg)
{
 ...
 int Robot::navigation(MoveBaseClient *ac, int num)
{
  ...
  while (waypoint[num][2] !=777) {
    ...
    ROS_INFO("Sending the next waypoint No.%d",num+1);
    ac->sendGoal(goal);
    bool result = false;
    ...
    while (!result) {
      ...
      if (dist <= 0.2 && diff_yaw <= 0.6 /* * M_PI/180*/) {
	result = true;
      }
      usleep(5*1000);
    }
    ...
    if (result) {
      ...
      ROS_INFO("Arrived at waypoint No.%d",num+1);
      stringstream s_num;
      if (num != 0) {
	s_num << num;
	string msg;
    ...
    switch (num) {
    case 1: case 2:  {
      ROS_INFO("Go to the next waypoint No.%d",num+1);
      stringstream s_num2;
      s_num2 << num+1;
      string msg2;
      msg2 = "I go to the waypoint" + s_num2.str();
      sc->say(msg2);
      usleep(2000*1000);
      break;
    }
    case 3: 
      ROS_INFO("Follow person state");
      sc->say("Follow person state");
      usleep(2000*1000);
      break;
    case 4:
      ROS_INFO("I go out of this room");
      sc->say("I go out of this room");
      usleep(2000*1000);
      break;
    default:
      ROS_INFO("I completed the navigation test");
      sc->say("I completed the navigation test");
      usleep(2000*1000);
    }
    ...
    if (num == 1) return WP1_STATE;
    else if (num == 2) return WP2_STATE;
    else if (num == 3) return WP3_STATE;
    else if (num == 5) return GOAL_STATE;
...
void Robot::init()
{
  ...
  // Set the subscribers
  speechSubscriber = nh.subscribe("/recognizer/output",1,&Robot::speechCallback,this);
...
int main(int argc, char* argv[])
{
  ...
  while(ros::ok()) {
    ...
    case NAVI_STATE: { 
      int state;
      state = robot.navigation(&ac, wp_num); 
      robot.setState(state); // 複数の状態に遷移する
      break;
    }
    case MOVE_PERSON_STATE: 
      sc->say("Please move out");
      usleep(3000*1000);
      robot.setState(NAVI_STATE);
      break;                                             
    case MOVE_OBJECT_STATE: 
      sc->say("Move out"); // あとで物体を移動する関数を実装
      usleep(3000*1000);
      robot.setState(NAVI_STATE);
      break;                                             
    case FOLLOW_STATE: {
  ...
  /* while (wp[wp_num][2] !=777) {
    ...
    ac.endGoal(goal);
    bool result = ac.waitForResult(ros::Duration(300.0));

    double x = robot.robotPose.pose.position.x;
    double y = robot.robotPose.pose.position.y;
    ROS_INFO("WP(%.1f,%.1f) Pos(%.1f,%.1f)\n",
  ...
  */  
  ...
  /* 
  // エレベータからやりたいときはコメントアウト
  int loop = 0;
  robot.setState(STATE_BEGIN);
  while(ros::ok()) {
    // Measure time begin 
    int64 time = cv::getTickCount();
    robot.prepWindow();
    ...
    switch (robot.getState()) {
      case STATE_BEGIN: robot.memorizeOperator();     break;
      case STATE1:      robot.followPerson(e1_img); break;
      case STATE2:      robot.actionInElevator();     break;
      case STATE3:      robot.searchOperator();       break;
      case STATE_END:   robot.stop();                 break;
      case STATE_TEST:  robot.test();                 break;           
      default: cout << "error state" << endl;         break;
    }
